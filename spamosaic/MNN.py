...
def nn_approx(ds1, ds2, names1, names2, knn=50):
    ...
    ----------
    ds1 : np.ndarray
        Query dataset of shape ``(N1, D)``.
    ds2 : np.ndarray
        Reference dataset of shape ``(N2, D)``.
    names1 : list of str
...
def nn_annoy(ds1, ds2, names1, names2, norm=True, knn=20, metric='euclidean', n_trees=10, save_on_disk=False):
    ...
    norm : bool, default=True
        Whether to L2-normalize datasets before indexing/search.
    knn : int
        Number of nearest neighbors to retrieve.
    metric : str, default='euclidean'
        Distance metric (e.g., ``'euclidean'``, ``'manhattan'``).
    n_trees : int, default=10
        Number of trees to build in the Annoy index.
    save_on_disk : bool, default=False
        If ``True``, write the index to disk.
...
def mnn(ds1, ds2, names1, names2, knn1=20, knn2=20,
    ...
    ----------
    ds1 : np.ndarray
        First dataset (queries), shape ``(N1, D)``.
    ds2 : np.ndarray
        Second dataset (references), shape ``(N2, D)``.
    names1 : list of str
        Identifiers for rows in ``ds1``.
    names2 : list of str
        Identifiers for rows in ``ds2``.
    ...
    approx : bool, default=True
        If ``True``, use approximate search (HNSW/Annoy); otherwise exact kNN.
    metric : str, default='euclidean'
        Distance metric used when ``way='annoy'``.
    way : str, default='hnsw'
        Approximation backend: ``'hnsw'`` or ``'annoy'``.
    norm : bool, default=False
        Whether to normalize inputs before Annoy search (ignored for HNSW/exact).
    ...
    if approx:
        if way == 'hnsw':
            ...
            # output KNN points for each point in ds1; size ~ N1 * knn1
