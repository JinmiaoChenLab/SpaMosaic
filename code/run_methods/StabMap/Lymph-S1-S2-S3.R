...
data_dir = '../../../data/processed/Lymph-S1-S2-S3/'
out_dir = '../../../results/embeddings/StabMap/Lymph-S1-S2-S3/'
...
test_other_count = read_mat(pj(data_dir, 'R/S3', 'adt'))
...
# avoid all-zero expression
sf <- scuttle::librarySizeFactors(mult_other)
sf[is.na(sf) | sf == 0] <- 1
SingleCellExperiment::sizeFactors(mult_other) <- sf
mult_other <- scuttle::logNormCounts(mult_other)
