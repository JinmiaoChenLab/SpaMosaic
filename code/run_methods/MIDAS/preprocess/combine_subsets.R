...
merge_counts <- function(mod) {
    ...
    for (dataset_id in seq_along(input_dirs)) {
        ...
        if (file.exists(fp)) {
            ...
            sc <- LoadH5Seurat(fp)
            if (mod=='rna'){
                sc = RenameAssays(sc, assay.name='rna.tmp', new.assay.name='rna')
            }else{
                sc = RenameAssays(sc, assay.name='adt.tmp', new.assay.name='adt')
            }
...
merge_frags <- function() {
    ...
    # rownames(sc_merge[[mod]]@counts) <- feat_merged  # correct feature names for count data
    # sort features
    merged_counts = sc_merge[[mod]]@counts
    rownames(merged_counts) = feat_merged
    merged_assay <- CreateChromatinAssay(counts = merged_counts, sep = c(":", "-"), min.features = 0)
    new_sc_merge <- CreateSeuratObject(merged_assay, assay = "atac")
    new_sc_merge@meta.data = sc_merge@meta.data 

    gr_sorted <- sort(StringToGRanges(feat_merged))
    feat_dims[[mod]] <- width(gr_sorted@seqnames)   # fuck this
    ...
    sc_split <- SplitObject(new_sc_merge, split.by = "subset_id")
    for (subset_id in names(sc_split)) {
        prt("Saving subset ", subset_id, " ...")
        output_dir <- pj(task_dir, paste0("subset_", subset_id))
        output_mat_dir <- pj(output_dir, "mat")
