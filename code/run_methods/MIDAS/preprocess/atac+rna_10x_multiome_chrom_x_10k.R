...
base_dir <- "data/raw/atac+rna/10x_multiome_chrom_x_10k"
count_path <- pj(base_dir, "10k_PBMC_Multiome_nextgem_Chromium_X_filtered_feature_bc_matrix.h5")
frag_path <- pj(base_dir, "10k_PBMC_Multiome_nextgem_Chromium_X_atac_fragments.tsv.gz")
...
atac <- subset(atac, subset =
    nCount_atac > 500 & nCount_atac < 5e4 &
    nucleosome_signal < 2 &
    TSS.enrichment > 2
)
