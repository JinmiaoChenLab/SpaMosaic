...
base_dir <- "data/raw/atac+rna/10x_multiome_arc2_10k"
count_path <- pj(base_dir, "pbmc_granulocyte_sorted_10k_filtered_feature_bc_matrix.h5")
frag_path <- pj(base_dir, "pbmc_granulocyte_sorted_10k_atac_fragments.tsv.gz")
...
atac <- subset(atac, subset =
    nCount_atac > 800 & nCount_atac < 4e4 &
    nucleosome_signal < 2 &
    TSS.enrichment > 2
)
