...
def gen_all_batch_ids(s_joint, combs):
    ...
    for subset, comb in enumerate(combs):  # 这个for循环对每个batch一个编号，{'rna': batch_id, 'joint':batch_id, ...}
        s_subset = {}                      # 
        for m in comb+["joint"]:
    ...
    dims_s = {}
    for m in list(np.unique(flat_lists(combs)))+["joint"]:  # 遍历每个模态
        s_m = []                                            # 重新编码每个batch内的batch id为当前模态内的batch序列
        for s_subset in s:
